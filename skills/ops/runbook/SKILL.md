...
| Incident response (final) | No | `ops/final/incident-response-final.md` — severity levels, escalation paths |
| Architecture (final) | No | `design/final/architecture-final.md` — service dependencies for impact analysis |
| Database design (final) | No | `design/final/database-final.md` — database operations procedures |
| CI/CD pipeline (final) | No | `deploy/final/cicd-pipeline-final.md` — deployment/rollback procedures |
...
| Input | Required | Source |
|-------|----------|--------|
| Existing runbooks draft | Yes | `draft/runbooks-draft.md` or `draft/runbooks-v{N}.md` |
...
|------|------------|----------|
| Create | `runbooks-draft.md` | `draft/` |
| Refine | `runbooks-v{N}.md` | `draft/` (N = next version number) |

When user is satisfied -> they copy from `draft/` to `ops/final/runbooks-final.md`.
...
- User runs `/ops-runbook-refine` AND existing draft exists in `draft/` -> **Mode 2 (Refine)**
...
8. Write improved version to `draft/runbooks-v{N}.md`
...
- **Create mode**: Write to `draft/runbooks-draft.md`
- **Refine mode**: Write to `draft/runbooks-v{N}.md`, include Change Log and Diff Summary

Tell the user:
> **Operational Runbooks {created/refined}!**
> - Output: `draft/runbooks-{version}.md`
> - Readiness: {verdict}
...
> - When satisfied, copy to `ops/final/runbooks-final.md`
