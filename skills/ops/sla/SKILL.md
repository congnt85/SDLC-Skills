...
description: >
  ...
  level targets. ONLY activated by command: `/ops-sla`. Use `--create`, `--refine`, or `--score` to set mode. NEVER auto-trigger based on keywords.
...
| Input | Required | Source |
|-------|----------|--------|
| Artifact to score | Yes | `sdlc/ops/draft/sla-spec-draft.md` or latest `sla-spec-v{N}.md` or `sdlc/ops/final/sla-spec-final.md`, or user-specified path |
...
| Score | `sla-spec-scoreboard.md` | `sdlc/ops/draft/` |
...
2. Exists in sdlc/ops/final/sla-spec-final.md?             → Read it → DONE
3. Exists as sdlc/ops/draft/sla-spec-v{N}.md (latest N)?   → Read it → DONE
4. Exists as sdlc/ops/draft/sla-spec-draft.md?             → Read it → DONE
...
- Write to `sdlc/ops/draft/sla-spec-scoreboard.md`

Tell the user:
> **Scoreboard complete!**
> - Output: `sdlc/ops/draft/sla-spec-scoreboard.md`
> - Average: {avg}/5 — {verdict}
> - Lowest: {dimension} ({score}/5)
> - Issues: {N} (HIGH: {H}, MED: {M}, LOW: {L})
>
> **Next steps:**
> - Run `/ops-sla --refine` to address issues
> - Or run `/skill-evolution --analyze ops/sla` to improve the skill definition itself
