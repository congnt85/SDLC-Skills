...
description: >
  ...
  command: `/ops-change`. Use `--create`, `--refine`, or `--score` to set mode. NEVER
...
| Input | Required | Source |
|-------|----------|--------|
| Artifact to score | Yes | `sdlc/ops/draft/change-mgmt-draft.md` or latest `change-mgmt-v{N}.md` or `sdlc/ops/final/change-mgmt-final.md`, or user-specified path |
...
| Score | `change-mgmt-scoreboard.md` | `sdlc/ops/draft/` |
...
2. Exists in sdlc/ops/final/change-mgmt-final.md?             → Read it → DONE
3. Exists as sdlc/ops/draft/change-mgmt-v{N}.md (latest N)?   → Read it → DONE
4. Exists as sdlc/ops/draft/change-mgmt-draft.md?             → Read it → DONE
...
- Write to `sdlc/ops/draft/change-mgmt-scoreboard.md`

Tell the user:
> **Scoreboard complete!**
> - Output: `sdlc/ops/draft/change-mgmt-scoreboard.md`
> - Average: {avg}/5 — {verdict}
> - Lowest: {dimension} ({score}/5)
> - Issues: {N} (HIGH: {H}, MED: {M}, LOW: {L})
>
> **Next steps:**
> - Run `/ops-change --refine` to address issues
> - Or run `/skill-evolution --analyze ops/change` to improve the skill definition itself
